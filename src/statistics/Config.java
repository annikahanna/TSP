...
public enum Config {
    ...
    private List<Integer> getFitnessDataFromDB() {
        ...
        ResultSet rs = HSQLDBManager.instance.selectData("Select * from data");
        try {
            while(rs.next()) {
    ...
    public String buildFileBeginning(List<Integer> scenario_ids, String template_path) throws IOException {
        ...
        text = text.replaceAll(Config.VAR_DATADIR,Config.instance.path);
        text = text.replaceAll(Config.VAR_SCENARIODESCRIPTION,Config.instance.writeCsvInScenarios(scenario_ids));
        return text;
    }
